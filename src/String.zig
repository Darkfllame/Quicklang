...
pub fn format(self: String, comptime _: []const u8, _: std.fmt.FormatOptions, stream: anytype) !void {
    try stream.print("{?s}", .{if (self.buffer) |_| self.str() else null});
}
...
pub fn hash(self: String) usize {
    return hashOf(self.str());
}
pub fn hashOf(s: []const u8) usize {
    var h: usize = 5381;
